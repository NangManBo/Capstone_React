...
function MainPage() {
  ...
  // 카테고리 상수
  const categories = ['시사', '정치', '게임', '스포츠', '음식', '반려동물'];
  // 투표 데이터를 받아오는 함수
  useEffect(() => {
        ...
        if (response.status === 200) {
          ...
          if (Array.isArray(votesData)) {
            const formattedVotes = votesData.map(vote => ({
              ...
              createdAt: moment.utc(vote.createdAt).format('YYYY-MM-DD HH:mm'),
            ...
            console.error('Invalid votes data format:', votesData);
          }
        } else {
  ...
  const getCategoryVotes = () => {
    return categories.map(category => {
      ...
      const matchingVotes = votes.filter(vote => vote.category === category);
      ...
      return (
        <button
          key={`${category}-${firstMatchingVote?.title || ''}`}
